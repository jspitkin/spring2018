...
/* Given two strings, joins them into one with a space between them.
 * */
char *str_join(char *s1, char *s2) {
    char *s3 = malloc(strlen(s1) + strlen(s2) + 2);
    strcpy(s3, s1);
    strcat(s3, " ");
    strcat(s3, s2);
    return s3;
...
void file_checksum(int index) {
    ...
    if (st.st_size < 1) {
	char empty_checksum[8] = "00000000";
	file_names[index] = str_join(file_names[index], empty_checksum);
	return;
    }
    ...
    uint32_t checksum = crc32(0, file_data, st.st_size);	 
    char checksum_hex[8];
    sprintf(checksum_hex, "%08X", (int)checksum);
    file_names[index] = str_join(file_names[index], checksum_hex);
